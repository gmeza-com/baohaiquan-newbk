...
@push('header')
<script>
    CNV.categoryActive = '/danh-muc?id=' + {{ $category->category->id }};
</script>
@endpush
...
<section id="include">
    <div class="container">
        <div class="row ">
            <div class="col-sm-8">
                @include('theme::partial.breadcum', ['title' => $category->name, 'url' => $category->link])
                @foreach($posts as $post)
                <div class="wrap-news-list destop-row" data-wow-delay="0.1s">
                    <div class="col-sm-5 img-news">
                        <div class="img-ctxh">
                                <a href="{{ $post->language('link') }}" title="{{ $post->language('name') }}">
                                    <div class="mask">
                                        <div>
                                    ...
                                    <div class="info-time">
                                        ...
                                        <p>T{{ $post->published_at->format('m') }}</p>
                                    </div>
                                </a>
                            </div>
                    </div>
                    <div class="col-sm-7 quote-news destop-pr0">
                        <h3><a href="{{ $post->language('link') }}">{{ $post->language('name') }}</a></h3>
