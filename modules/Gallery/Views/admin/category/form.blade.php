                            ...
                            <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="{{ $language['locale'] }}">
                                ...
                                @if(config('cnv.seo_plugin'))
                                    <div class="form-group">
                                        ...
                                        <textarea name="language[{{ $language['locale'] }}][description]" id="description" class="form-control" rows="10" required>{{ $category->seo ? @$category->seo->language('description', $language['locale']) : null }}</textarea>
                                    </div>
                                @endif
                            </div>
                        @endforeach
