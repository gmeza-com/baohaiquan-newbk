...
@section('content')
    ...
    @component('components.block')
        ...
        <div class="text-center">
            ...
            <a href="{{ request()->url() }}" class="btn btn-default btn-alt {{  !request()->has('published') ? 'active' : '' }}">
                Tất cả
            </a>
            <a href="{{ request()->url() }}?published=-1" class="btn btn-danger btn-alt {{  request()->has('published') && request('published') == -1 ? 'active' : '' }}">
                Đã hủy
            </a>
            <a href="{{ request()->url() }}?published=0" class="btn btn-warning btn-alt {{  request()->has('published') && request('published') == 0 ? 'active' : '' }}">
                {{ trans('news::language.waiting_level_1') }}
            </a>
            @if(allow('news.post.approved_level_2') || allow('news.post.approved_admin'))
            <a href="{{ request()->url() }}?published=1" class="btn btn-info btn-alt {{ request('published') == 1 ? 'active' : '' }}">
                {{ trans('news::language.waiting_level_2') }}
            </a>
            @endif
            <a href="{{ request()->url() }}?published=2" class="btn btn-primary btn-alt {{ request('published') == 2 ? 'active' : '' }}">
                {{ trans('news::language.waiting_level_3') }}
            </a>
            <a href="{{ request()->url() }}?published=3" class="btn btn-success btn-alt {{ request('published') == 3 ? 'active' : '' }}">
