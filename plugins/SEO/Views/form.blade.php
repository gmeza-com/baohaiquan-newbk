                    ...
                    <div class="tab-content" style="padding: 16px">
                        @foreach (config('cnv.languages') as $language)
                            <div class="tab-pane {{ $loop->first ? 'active' : '' }}" id="seo-{{ $language['locale'] }}">
                                <h4 class="seo_plugin_title" data="title_{{ $language['locale'] }}">
                    ...
                    <div class="tab-content">
                        @foreach (config('cnv.languages') as $language)
                            <div class="tab-pane {{ $loop->first ? 'active' : '' }}"
                                id="seo-edit-{{ $language['locale'] }}">
                                <div class="form-group">
                                    {!! Form::label('title', trans('seo_plugin::language.title'), ['class' => 'label-control col-md-3']) !!}
                                    <div class="col-md-9">
                                        {!! Form::text(
                                            'seo[language][' . $language['locale'] . '][title]',
                                            @$model->seo ? @$model->seo->language('title', $language['locale']) : null,
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-9">
                                        {!! Form::textarea(
                                            'seo[language][' . $language['locale'] . '][description]',
                                            @$model->seo ? @$model->seo->language('description', $language['locale']) : null,
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-9">
                                        {!! Form::text('language[' . $language['locale'] . '][slug]', @$model->language('slug', $language['locale']), [
                                            'class' => 'form-control',
                                            'required',
                                        ]) !!}
                                        <p class="help-block">
            ...
            (function($) {
                ...
                @foreach (config('cnv.languages') as $language)
                    $('[name="language[{{ $language['locale'] }}][name]"]').keyup(function() {
                        ...
                        $('[name="seo[language][{{ $language['locale'] }}][title]"]').val($(this).val().substr(0,
                            69));
